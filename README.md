...
**`cart:add`**
- Генерируется: `ProductPreview` при клике на кнопку «В корзину».
- Данные: `{ id: string }` – идентификатор добавляемого товара.
- Назначение: добавить товар в модель корзины и обновить отображение.
...
- Данные: `{ payment: 'cash' | 'card', address: string }` – данные формы заказа.
...
#### События загрузки данных (презентер)

Эти события генерируются презентером при работе с сервером (не являются частью представлений).

**`products:loaded`**
- Генерируется: презентер после успешной загрузки товаров с сервера.
- Данные: `{ items: IProduct[] }` – массив загруженных товаров.
- Назначение: сохранить товары в модель каталога и отобразить их в галерее.

**`order:success`**
- Генерируется: презентер после успешного оформления заказа.
- Данные: `{ id: string, total: number }` – идентификатор и сумма заказа.
- Назначение: показать окно успешного оформления.

**`order:error`**
- Генерируется: презентер при ошибке оформления заказа.
- Данные: `{ error: string }` – текст ошибки.
- Назначение: показать сообщение об ошибке пользователю.
...
- Данные: `IBuyerValidationErrors` – объект с ошибками валидации.
- Назначение: включить/выключить кнопку отправки формы, показать ошибки.

### Презентер

Презентер реализован непосредственно в основном скрипте `main.ts`.
