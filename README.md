...
- `IProduct` – товар (id, название, изображение, категория, цена, описание).
- `IBuyer` – покупатель (способ оплаты, адрес, email, телефон).

**Типы для обмена с сервером:**
- `IProductsResponse` – ответ сервера со списком товаров: `{ items: IProduct[], total?: number }`.
- `IOrderData` – данные для отправки заказа: `{ payment: TPayment, email: string, phone: string, address: string, items: string[] }`.
- `IOrderResponse` – ответ сервера после оформления заказа: `{ id: string, total: number }`.

**Вспомогательные типы:**
- `TPayment` – возможные способы оплаты (`'cash'` | `'card'`).
...
- `getTotal(): number` – вычисляет общую стоимость товаров в корзине (товары с ценой `null` считаются как 0).
